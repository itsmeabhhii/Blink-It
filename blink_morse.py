...
from_whatsapp_number=credentials.from_whatsapp_number
to_whatsapp_number=credentials.to_whatsapp_number

webbrowser.open("morse_tester.txt")
time.sleep(1)

# handle = gw.getWindowsWithTitle('BlinkIt')[0]
# handle.activate()
# time.sleep(1)
# keyboardPynput.press(Key.cmd.value)
# keyboardPynput.press(Key.down.value)
# keyboardPynput.release(Key.down.value)
# keyboardPynput.release(Key.cmd.value)
# time.sleep(1)
# handle = gw.getWindowsWithTitle('Notepad')[0]
# handle.activate()
# time.sleep(1)
# keyboardPynput.press(Key.cmd.value)
# keyboardPynput.press(Key.left.value)
# keyboardPynput.release(Key.left.value)
# keyboardPynput.release(Key.cmd.value)
...
def main():
	...
	handle.activate()
	time.sleep(1)
	keyboardPynput.press(Key.ctrl.value)
	keyboardPynput.press(Key.end.value)
	keyboardPynput.release(Key.end.value)
	keyboardPynput.release(Key.ctrl.value)
	time.sleep(1)

	total_morse = loop_camera(vs, detector, predictor, lStart, 
			lEnd, rStart, rEnd)
	# loop_camera(vs,detector,predictor,lStart,lEnd,rStart,rEnd)
	cleanup(vs)
		...
		for rect in rects:
			...
			# threshold
				...
				# detected and eyes open for > PAUSE_CONSEC_FRAMES.
					...
					CLOSED_EYES = False
					EYES_OPEN_COUNTER = 0
					if initiated and morse_code.from_morse(morse_word) == 'B':
						#python slicing using positive index
						l = len(txt)
						txt = txt[:l-1]
						print("in Backspace mode")
						#print('\b ', end="", flush=True) 
						#sys.stdout.write('\b')
						#keyboardPynput.press('k')
						keyboardPynput.press(Key.backspace)
						#keyboardPynput._Key.backspace
					elif initiated and morse_code.from_morse(morse_word) == 'C':
						...
						time.sleep(1)
						keyboardPynput.press(Key.ctrl.value)
						keyboardPynput.press('c')
						keyboardPynput.release('c')
						keyboardPynput.release(Key.ctrl.value)
						time.sleep(2)
					elif initiated:
						keyboard.write(morse_code.from_morse(morse_word))
						txt+=morse_code.from_morse(morse_word)
					if initiated and morse_code.from_morse(morse_word) == '.':
						print("##")
						print(txt)
						client.messages.create(body=txt,
                        					   from_=from_whatsapp_number,
                        					   to=to_whatsapp_number)
						print("##")
					if not initiated and morse_code.from_morse(morse_word) == 'S':
						morse_word += "S"
						total_morse += "S"
						morse_char += "S"
						initiated=True
				...
				elif (initiated and WORD_PAUSE and EYES_OPEN_COUNTER >= 
